...
WIKI_ARTICLES = [
    {
        ...
        "category": "debutants",
        "content": "Bitcoin est la première cryptomonnaie décentralisée, créée en 2009 par Satoshi Nakamoto. C'est une monnaie numérique qui fonctionne sans banque centrale, utilisant la technologie blockchain pour enregistrer toutes les transactions de manière sécurisée et transparente.",
        "tags": ["bitcoin", "base", "blockchain"]
    },
    {
        "id": "what-is-blockchain",
        "title": "C'est quoi la Blockchain ?",
        "category": "debutants",
        "content": "La blockchain est un registre numérique distribué qui enregistre toutes les transactions de manière chronologique et immuable. Imaginez un grand livre de comptes partagé par des milliers d'ordinateurs dans le monde, où chaque transaction est vérifiée par le réseau.",
        "tags": ["blockchain", "base", "technologie"]
    },
    {
        "id": "how-to-buy-crypto",
        "title": "Comment acheter sa première crypto ?",
        "category": "debutants",
        "content": "1. Choisissez une plateforme d'échange réputée (Binance, Coinbase, Kraken)\n2. Créez un compte et vérifiez votre identité\n3. Déposez des fonds (virement bancaire ou carte)\n4. Achetez votre première crypto\n5. Transférez vers un wallet sécurisé si vous gardez longtemps",
        "tags": ["achat", "exchange", "débutant"]
    },
    {
        "id": "common-scams",
        "title": "Comment éviter les arnaques ?",
        "category": "securite",
        "content": "⚠️ Signaux d'alerte:\n- Promesses de gains garantis\n- Pression pour investir rapidement\n- Équipe anonyme\n- Pas d'audit de smart contract\n- Prix artificiellement gonflé\n\n✅ Bonnes pratiques:\n- Faites vos propres recherches (DYOR)\n- Ne partagez jamais vos clés privées\n- Méfiez-vous des messages privés non sollicités",
        "tags": ["scam", "sécurité", "protection"]
    },
    {
        "id": "what-is-wallet",
        "title": "Qu'est-ce qu'un Wallet crypto ?",
        "category": "debutants",
        "content": "Un wallet (portefeuille) crypto est un outil pour stocker, envoyer et recevoir des cryptomonnaies. Il existe deux types:\n\n🔥 Hot Wallet: Connecté à internet (Metamask, Trust Wallet)\n❄️ Cold Wallet: Hors ligne, plus sécurisé (Ledger, Trezor)\n\nVotre wallet contient vos clés privées - ne les partagez JAMAIS !",
    ...
    },
    {
        ...
        "category": "trading",
        "content": "DCA (Dollar Cost Averaging) = Investir un montant fixe à intervalles réguliers.\n\nExemple: 100€ chaque mois dans Bitcoin, peu importe le prix.\n\n✅ Avantages:\n- Réduit l'impact de la volatilité\n- Pas besoin de timer le marché\n- Discipline automatique\n- Moins de stress émotionnel",
        "tags": ["DCA", "stratégie", "investissement"]
    }
]
...
async def get_community_posts(
    ...
    posts = await db.community_posts.find(query, {"_id": 0}).sort(sort_field).skip(skip).limit(limit).to_list(limit)
    ...
    for post in posts:
        ...
        comments_count = await db.community_comments.count_documents({"post_id": post["id"]})
...
@api_router.get("/community/bookmarks")
...
@api_router.post("/pro/upload-file")
...
async def get_community_post(post_id: str):
    ...
    # Get comments
    comments = await db.community_comments.find({"post_id": post_id}, {"_id": 0}).sort("votes", -1).to_list(100)
    enriched_comments = []
    for comment in comments:
        comment_author = await db.users.find_one({"id": comment["author_id"]}, {"_id": 0})
...
async def vote_post(
    ...
    existing_vote = await db.community_votes.find_one({
        "user_id": current_user["id"],
        "post_id": post_id
    })
    ...
    if existing_vote:
        ...
        if old_vote == vote_change:
            # Remove vote
            await db.community_votes.delete_one({"_id": existing_vote["_id"]})
        ...
        # New vote
        await db.community_votes.insert_one({
            "id": str(uuid.uuid4()),
            "user_id": current_user["id"],
            "post_id": post_id,
...
async def report_community_post(post_id: str, report_data: dict = Body(...), current_user: dict = Depends(get_current_user)):
...
@api_router.get("/community/user/{user_id}/posts")
async def get_user_posts(user_id: str):
    """Get posts by a specific user"""
    posts = await db.community_posts.find({"author_id": user_id}, {"_id": 0}).sort("created_at", -1).to_list(50)
...
async def get_community_leaderboard():
    ...
    users = await db.users.find({"community_score": {"$gt": 0}}, {"_id": 0}).sort("community_score", -1).limit(10).to_list(10)
...
async def get_user_detail(user_id: str, admin_user: dict = Depends(get_admin_user)):
    ...
    comments_count = await db.community_comments.count_documents({"author_id": user_id})
...
async def promote_user(
    ...
    await db.users.update_one({"id": user_id}, {"$set": {"role": request.role}})
...
@api_router.put("/admin/users/{user_id}/set-vip")
...
@api_router.put("/admin/users/{user_id}/set-pro")
async def admin_set_pro(
    ...
    if is_pro:
        ...
        existing_profile = await db.pro_profiles.find_one({"user_id": user_id})
...
async def admin_delete_post(post_id: str, admin_user: dict = Depends(get_admin_user)):
    ...
    await db.community_votes.delete_many({"post_id": post_id})
...
async def admin_delete_comment(comment_id: str, admin_user: dict = Depends(get_admin_user)):
    ...
    await db.community_votes.delete_many({"comment_id": comment_id})
...
async def get_admin_posts(
    ...
    enriched_posts = []
    for post in posts:
        author = await db.users.find_one({"id": post["author_id"]}, {"_id": 0, "password_hash": 0})
        comments_count = await db.community_comments.count_documents({"post_id": post["id"]})
        post_data = {k: (v.isoformat() if isinstance(v, datetime) else v) for k, v in post.items()}
        enriched_posts.append({
            **post_data,
...
async def get_admin_stats(admin_user: dict = Depends(get_admin_user)):
    ...
    for i in range(6, -1, -1):
        ...
        comment_count = await db.community_comments.count_documents({
            "created_at": {"$gte": day_start, "$lt": day_end}
        })
    ...
    admin_only = await db.users.count_documents({"role": "admin"})
    ...
    categories = await db.community_posts.aggregate([
